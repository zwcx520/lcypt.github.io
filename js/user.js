var myuserhost = "my.3dmgame.com";
if (typeof (layer) != "undefined") {
    a = document.getElementsByTagName("head")[0],
        s = document.createElement("link");
    s.rel = "stylesheet";
    s.href = myuserhost + "/m/css/third/haslayer.css?v=3.1.01";
    a.appendChild(s);
} else {
    ...
    ;!function (e, t) {
        ...
        var i, n, a = e.layui && layui.define, o = {
            ...
            v: "3.1.0", ie: function () {
                ...
                var t = "layer", i = "", n = "/m/css/third/layer.css?v=" + r.v + i;
...
var g_login = 0, g_logout = 0, g_register = 0, recomcode = '', g_regcaptcha = 0;
$(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 1) {
                    ...
                    userinfo += '<a href="' + myuserhost + '/setting"><div class="name">' + data.user.nickname + '</div></a>' + (typeof (data.user.hasmsg) == "undefined" || data.user.hasmsg != 1 ? '' : '<i class="tx_box"></i>');
                    userinfo += '<a href="javascript:void(0);" class="out_btn" onclick="logout_submit()">退出</a>';
                    if(data.data.synurl){
                        userinfo += '<img src="'+data.data.synurl+'" style="width:0px; height:0px;border:none;" />';
                    }
                    userinfo += '</div>';
                    $('.landing').html(userinfo);
                    //需要刷新评论的地方
                    try {
...
function logout_submit() {
    ...
    if(ishttps){
        ...
        window.location.href = myuserhost+"/api/logouthandle?redirect="+encodeURIComponent(window.location);
    }
}
...
function openlogin() {
    ...
    if ($("#my_login_form .login").is(':hidden')) {
        $("#my_login_form .login").show();
    }
    if ($("#my_login_form .registered").is(':visible')) {
        $("#my_login_form .registered").hide();
    }
}
...
function openregister() {
    ...
    if ($("#my_login_form .login").is(':visible')) {
        $("#my_login_form .login").hide();
    }
    if ($("#my_login_form .registered").is(':hidden')) {
        $("#my_login_form .registered").show();
    }
}
...
function login_submit() {
    ...
    $.ajax({
        url: url,
        type: "POST",
        data: {username: username, passwd: pwd, autologin: 1},
        dataType: 'json',
        xhrFields: {withCredentials: true},
        success: function (data) {
            if (data.code == 1) {
                ...
                try {
                    let ishttps = 'https:' == document.location.protocol ? true : false;
                    if (ishttps) {
                        //if(data.data != undefined && data.data.synurl != undefined && data.data.synurl != ""){
                        //  window.location.href = data.data.synurl+"&refer="+encodeURIComponent(window.location.href);
                        //  return false;
                        //}else{
                            window.location.reload();
                            return false;
...
function checkmobile(dom, mobile) {
    var url = myuserhost + "/api/checkmobile";
...
function get_regsms() {
    ...
    //判断是否点击过
    if ($('#tencentcaptcha').hasClass('time_bg')) {
        return;
    }
    var pwd = $('#regpasswd');
...
function reg_submit() {
    ...
    var cpwd = $('#checkpasswd');
    var mobile = $('#mobile');
    var remember_me = $('.common_chk').prop('checked') ? '1' : '0';
    ...
    var url = myuserhost + "/api/register";
    g_register = 1;
    $.ajax({
        url: url,
        type: "POST",
...
function qq_login() {
    ...
    if ($("#myreferurl").length > 0) {
        refererUrl = $("#myreferurl").val();
    } else {
        refererUrl = getParentUrl();
    }
    try {
        //加上游戏盒子判断
        if (refererUrl.indexOf('m.3dmgame.com/yeyou/' != -1)
            || refererUrl.indexOf("http://mweb.3dmgame.com/") != -1
            || refererUrl.indexOf("mweb.3dmgame.com/") != -1
            || refererUrl.indexOf("m.3dmgame.com/bt/") != -1
            || refererUrl.indexOf("http://m.3dmgame.com/bt/") != -1
        ) {
            if (typeof (window.ATW) != 'undefined') {
                if (window.ATW.getVersion() == "android") {
                    window.ATW.qqLogin();
                    return false;
                }
            }
    ...
    var url = myuserhost + "/login/qq?recomcode=" + recomcode + "&referer=" + encodeURIComponent(refererUrl);
...
function wechat_login() {
    ...
    var url = myuserhost + "/login/wechat?recomcode=" + recomcode + "&referer=" + encodeURIComponent(refererUrl);
    window.open(url, '_top');
}
...
function sina_login() {
    ...
    var url = myuserhost + "/login/sina?recomcode=" + recomcode + "&referer=" + encodeURIComponent(refererUrl);
    window.open(url, '_top');
}
...
function hzqqlogin_callback(accesstoken) {
    var params = {accesstoken: accesstoken};
    var url = myuserhost + '/jsonph5login/qqauth';
    $.ajax({
        url: url,
        data: params,
        type: "GET",
...
function hzwxlogin_callback(wxcode) {
    var params = {wxcode: wxcode};
    var url = myuserhost + '/jsonph5login/wxauth';
    $.ajax({
        url: url,
        data: params,
        type: "GET",
...
function hzwblogin_callback(token) {
    var params = {token: token};
    var url = myuserhost + '/jsonph5login/wbauth';
    $.ajax({
        url: url,
        data: params,
        type: "GET",
...
function bthzwxlogin_callback(wxcode) {
    var params = {wxcode: wxcode};
    var url = myuserhost + '/jsonph5login/btwxauth';
    $.ajax({
        url: url,
        data: params,
        type: "GET",
